...
import com.wangrg.java_lib.java_util.HttpUtil;
...
public class Downloader implements OnProgressListener {
    ...
    private ArrayList<com.wangrg.java_lib.java_program.downloader.bean.Task> tasks;
    private com.wangrg.java_lib.java_program.downloader.db.LocalSqLiteDb db;
    /**
    ...
    public Downloader(String dbDir, String downloadPath, int threadNumber) {
        ...
        db = new com.wangrg.java_lib.java_program.downloader.db.LocalSqLiteDb(dbDir);
//        TODO 查询数据库并给tasks赋值
        tasks = db.queryTasks();
    ...
    public int createTask(String url, String fileName) throws SQLException {
        com.wangrg.java_lib.java_program.downloader.bean.Task task = new com.wangrg.java_lib.java_program.downloader.bean.Task(fileName, downloadPath, threadNumber, url, 0, 0, false);
...
//        TODO 创建downloadThread并根据文件长度分配每条线程的下载指标，threadId从0到threadNumber-1
        for (int i = 0; i < threadNumber; i++) {
            com.wangrg.java_lib.java_program.downloader.bean.ThreadInfo threadInfo = new com.wangrg.java_lib.java_program.downloader.bean.ThreadInfo();
...
//        TODO 根据taskId从tasks读取任务信息，从数据库读取线程信息，创建线程对象,添加到downloadThreads,最后启动
        com.wangrg.java_lib.java_program.downloader.bean.Task task = Util.getTask(taskId, tasks);
        ArrayList<com.wangrg.java_lib.java_program.downloader.bean.ThreadInfo> threadInfos = db.queryThreads(taskId);
        for (com.wangrg.java_lib.java_program.downloader.bean.ThreadInfo info : threadInfos) {
...
//        TODO 根据taskId找到所有线程并结束。注意要使用监听器，等线程跳出下载循环再保存信息到数据库
        ...
        for (final DownloadThread downloadThread : downloadThreads) {
            final com.wangrg.java_lib.java_program.downloader.bean.ThreadInfo threadInfo = downloadThread.getThreadInfo();
...
//        TODO 保存所有线程信息到数据库
        for (final DownloadThread downloadThread : downloadThreads) {
            downloadThread.stop(new DownloadThread.OnStopListener() {
                @Override
                public void onStop() {
                    com.wangrg.java_lib.java_program.downloader.bean.ThreadInfo info = downloadThread.getThreadInfo();
                    db.updateThreadProgress(info.getTaskId(), info.getThreadId(), info.getFinished());
                }
            });
        }
//        TODO 保存所有任务信息到数据库
        for (com.wangrg.java_lib.java_program.downloader.bean.Task task : tasks) {
