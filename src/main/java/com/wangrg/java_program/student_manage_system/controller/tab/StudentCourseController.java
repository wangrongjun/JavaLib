...
public class StudentCourseController implements StudentCourseContract.IStudentCourseController {
    ...
    public void clickSelectCourse(int courseId) {
        LogUtil.print("courseId: " + courseId);
    }

    private void startLoad() {
        view.loading("正在加载");
        new Thread(() -> {
            totalRecordCount = studentService.getCourseAllCount();// TODO 优化：如果换页，不用查询数目，只有刷新才需要
            if (totalRecordCount == 0) {
                view.loadFinish("没有数据");
            } else {
                Long studentId = userService.getStudentFromCache().getStudentId();
                courseList = studentService.getAllCourse(studentId, currentPageIndex, PAGE_SIZE);
